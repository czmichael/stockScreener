...
public class AmexStockPopulateService extends AbstractStockPopulateService {

	private static final String AMEX_PAGE_URL_PREFIX = 
					"http://www.advfn.com/amex/americanstockexchange.asp?companies=";
	
	public void populate() {
		Character curAlphabet = 'A', lastAlphabet = 'Z';
		StringBuilder amexPageUrl;
			try {
				// Loop through all companies listing page starting from letter 'A' 
				while (curAlphabet <= lastAlphabet) {
					amexPageUrl = new StringBuilder(AMEX_PAGE_URL_PREFIX + curAlphabet);
					Document doc = Jsoup.parse(getHtmlWebService().downloadHtml(amexPageUrl.toString()));
				...
				// that's OK, continue until finish.
			}
	}
