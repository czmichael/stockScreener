...
public class NasdaqStockPopulateService extends AbstractStockPopulateService {

	private static final String NASDAQ_PAGE_URL_PREFIX = 
					"http://www.advfn.com/nasdaq/nasdaq.asp?companies=";
	
	public void populate() {
		Character curAlphabet = 'A', lastAlphabet = 'Z';
		StringBuilder nasdaqPageUrl;
		// Loop through all companies listing page starting from letter 'A' 
		while (curAlphabet <= lastAlphabet) {
			nasdaqPageUrl = new StringBuilder(NASDAQ_PAGE_URL_PREFIX + curAlphabet);
			Document doc = Jsoup.parse(getHtmlWebService().downloadHtml(nasdaqPageUrl.toString()));
